...
exports.sentEmailviaForm = functions.https.onRequest((req, res) => {
...
exports.sentEmailDirect = functions.https.onRequest((req, res) => {
    ...
    return cors(req, res, () => {
        ...
        fetch(url, {
            ...
            //Alguma ação caso tudo ocorra corretamente      
            ...
            //Alguma ação caso ocorra um erro
           console.log('Erro ao tentar enviar',err); 
        });
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            console.log("[Site Agroecologia em Rede] Fim da função");
            if (error) {
                res.status(200).send("Site Agroecologia em Rede] Erro:", error.message);
            }
            console.log("[Site Agroecologia em Rede] Fim da função");
            let mensagem = "<h3 style='font-family:arial'>Mensagem enviada com sucesso</h3><h4>Obrigado por entrar em contato.</h4>";
            mensagem = mensagem + "<a href='https://site-de-agroecologia-new.web.app/'>Voltar para a página principal</a>";
